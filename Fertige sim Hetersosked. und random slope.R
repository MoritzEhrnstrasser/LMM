...
simulate_all_models <- function(isample, N, Clstr, all_pred) {
  ...
  simulate_noSlope <- function(N, Clstr, all_pred) {
    ...
    result_ws_cr <- list(
      ...
      p_value      = fit_ws_cr$results["x1", "p.val"],
      varcorr      = fit_ws_cr[["vcov"]]
    )
    result_ns_cr <- list(
      coefficients = fit_ns_cr$results[, "Estimate"],
      p_value      = fit_ns_cr$results["x1", "p.val"],
  ...
  results_noSlope  <- lapply(1:isample, function(i) simulate_noSlope(N, Clstr, all_pred))
  results_withSlope <- lapply(1:isample, function(i) simulate_withSlope(N, Clstr, all_pred))
  ...
  aggregate_results <- function(results, model_types = c("ws", "ns", "ws_cr", "ns_cr")) {
    ...
    for(mt in model_types) {
      ...
      # Aggregation der Varianz-Kovarianzen
      if (mt %in% c("ws", "ns")) {
        ...
        allVarcorr <- t(sapply(results, function(res) {
          if (!is.null(res[[mt]]$varcorr)) {
            return(as.vector(res[[mt]]$varcorr))
          } else {
            return(rep(NA, length(as.vector(results[[1]][[mt]]$varcorr))))
          }
        }, simplify = TRUE))
      }
 ...
 result_all <- simulate_all_models(isample = 5, N = 50, Clstr = 50, all_pred = c(0, 0))
